 ...
 while_bl
    {
        ...
        if (ttp->next == 0)
        {
            ...
            ret = (pm_size - ttp->end_loc >= size && pm_size -ttp->end_loc <= ts) ? qw : ret;
            break;
        }
        else 
        {
            printf("WAS QW->NEXT NULL %zu\n", qw->next == 0);
            ts = (qw->next->start_loc - ttp->end_loc >= size && ttp->next->start_loc - ttp->end_loc <= ts) ? ttp->next->start_loc - ttp->end_loc : ts;
            ret = (qw->next->start_loc - ttp->end_loc >= size && ttp->next->start_loc - ttp->end_loc <= ts) ? ttp : ret;
            qw = qw->next;
        }
    } // end while
