...
void fifo()
{
 ...
 printf("The distance of the traversed tracks is:"); print_trav();
...
shortest = (seq[0] < shortest) ? seq[0] : shortest;
seq_bool[0] = true;
int ret_index;
for_cyl {
    ret_index = (seq[i] < shortest && seq_bool[i] == false) ? i : ret_index;
shortest = (seq[i] < shortest && seq_bool[i] == false) ? seq[i] : shortest;
...
seq_bool[0] = true;
    ...
    traversal[seq_count] = seq[findShortestIndex()];

    distance += traversal[seq_count] - traversal[seq_count - 1];
    seq_count++;

    }//end while

 printf("Traversed sequence::"); print_trav();
    printf("The distance of the traversed tracks is: %d\n", distance);
}
