...
int getSpace(bl *gs)
{
    ...
    printf("GS --> ID--%d S--%d E--%d\n\n", gs->id, gs->start_loc, gs->end_loc);
    bl* qw = gs->next;
    if (qw != NULL) {
    printf("GS->NEXT ID--%d S--%d E--%d\n\n", qw->id, qw->start_loc, qw->end_loc);
    }
    if (qw == NULL)
    {
    ...
    else
    {
        ...
        return qw->start_loc - gs->end_loc;
    } // end else
} // end func
...
bl *best_fit(bl *btf, int size)
{
    ...
    {
        if (w)
        printf("CALL BF getS %d \n", w->next->id);
        int s = getSpace(w);
        if (s >= size && s <= ts)
        {
            ts = s;
            printf("BEST FIT IF TRUE FOR ID -> %d, SIZE --> %d\n", w->id, ts);
            ret = w;
            w = w->next;
        }
    } // end while
