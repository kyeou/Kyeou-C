...
void printProcs()
{
	...
	//and so on
		*/
	 //printf("PRINTING AMOUNT OF %d\n", numOfProcs);
	for (int i = 0; i < numOfProcs - 1; i++)
	{
		if (PCB[i].child != NULL) //if a child exists
		{
			// printf("\tPCB[%d]", *(PCB[i].child));
			printf("PCB[%d] is the parent of: ", i);
			for (int a = 0; a < numOfProcs - 1; a++)
			{
				if (PCB[a].parent == i)
				{
					if (a != i) // PCB[0] is the parent of PCB[0]
					{
						// printf("\tCHILD FOUND\t");
						printf("PCB[%d]\t", a);
					}
...
void create()
{
//what if i start newC at the child of the user inputted par
...
//	printf("\nparent noted\n");
	...
	while (PCB[newC].parent != -1)
	{
		...
		printf("newC %d -----> %zu -----> %d\n", newC, (PCB[newC].parent != -1), PCB[newC].parent );
		newC++;
...
/*
	for (int b = newC; b < numOfProcs; b++) {
		if (PCB[newC].parent != -1) {
			printf("newC %d -----> Parent ? %zu -----> %d\n", newC, (PCB[newC].parent != -1), PCB[newC].parent );
			newC++;
		}
	}
	*/
	...
	PCB[newC].parent = par;
	PCB[newC].child = NULL;
	/* create a new link containing the child's index q and append the link to the end of the linked list of PCB[par] */
	...
	else
	{
		...
		while (PCB[newLink].child != NULL) //checks if children, who are also parents, have children
		{
			 printf("\nWHILE REACHED ---- %d\n", newLink);
