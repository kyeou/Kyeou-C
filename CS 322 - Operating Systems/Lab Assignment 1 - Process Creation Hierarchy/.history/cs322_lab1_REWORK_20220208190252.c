...
void destroy(int *q)
{
    printf("%d", PCB[*q].children.iChild)
    if (PCB[*q].children.iChild == NULL)
    ...
    else
    {
        DC++;
        printf("   %d     ", DC);
        if (PCB[*(PCB[*q].children.iChild)].children.iChild != NULL)
        {
            destroy(PCB[*(PCB[*q].children.iChild)].children.iChild);
        }
        destroy(PCB[*q].children.iChild);
        PCB[*q].parent = -1;
        q = NULL;
        free(q);
    }
...
void destroyer()
{
    if (PCB != NULL)
    {
        ...
        {
            printf("\nDESTROY CALLED\n");
            destroy(PCB[par].children.iChild);
