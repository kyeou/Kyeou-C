...
bl *best_fit(int size)
{
    ...
    for (int a = 0; a < amt; a++)
    {
      ...
      printf("VALUES BEING CHECKED %d->NEXT %d\n", PHY_MEM[a].id, PHY_MEM[a].next->id);
        printf("CURR ID --> %d\n", PHY_MEM[a].id);
        ...
        if ((PHY_MEM[a].next) && PHY_MEM[a].end_loc - PHY_MEM[a].next->start_loc < ts && PHY_MEM[a].end_loc - PHY_MEM[a].next->start_loc > size)
        {
            printf("LAST IF TRUE\n");
...
for (int a = 0; a < amt; a++) {
    if (PHY_MEM[a].id == i) {
        printf("DEL ID CHECK --> %d", PHY_MEM[a].id);
        del = &PHY_MEM[a];
        break;
    }
}
