...
// takes a parent, indexed though its children and destroys it, use same style as printChildren
// destory needs to check both its child and NUllity of next
void destroy(int *child) // take a child, goes to PCB[child] takes out all train of nextxs
{                        // ah i might lose points for this
//next ends up being null has the next points to the address of another proces
    int nextCount = 0, NN;
    NN = PCB[*child].next->field;
    while (PCB[NN].next != NULL)
    {
        // set to next next lol
        NN = PCB[NN].next->field;
        nextCount++;
    }
