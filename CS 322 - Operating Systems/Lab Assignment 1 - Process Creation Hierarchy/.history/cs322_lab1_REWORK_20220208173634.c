...
void destroy(pcb p) {
    int a = *(p.children.iChild); //this is the index of child process
    if (p.children.iChild == NULL) {//if process has no children, the process should be freed in destroyer()
        return; 
    } else {//if process has children, those shits need to be set to NULL and freed
        //need to figure out how to deal with the children of the main process child
        if (PCB[a].children.iChild != NULL) {
            destroy(PCB[a].children.iChild);
        } //end of if
        destroy(PCB[*(p.children.next)]);
        PCB[*(p.children.iChild)].parent = 
        p.children.iChild = NULL;
        free(p.children.iChild);
