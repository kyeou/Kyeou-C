...
void create()
{
    ...
    { // this handles if an already existing child has a next
        ...
        printf("\nELSE 2 REACHED\n");
        newLink = PCB[*(PCB[par].child)].next->field; // This is terminating the program, this should not be NULL at this point
        // printf("\n The next field: ");
        // printf("%d\n", PCB[*(PCB[par].child)].next->field);
        //
        while (PCB[(PCB[newLink])].next != NULL)
        {
            printf("\nWHILE REACHED\n");
            newLink = PCB[*(PCB[newLink]).].next->field;
            rPoEn = PCB[*(PCB[newLink]).child].field;
        }
        printf("\nAFTER WHILE\n");
