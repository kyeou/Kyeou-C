...
int findShortestIndex(int seq_count)
{
    ...
    for_cyl
    {
      if ((seq[i] < shortest && seq_bool[i] == false)) {printf("TRUE CASE %d\n", i);}
        ret_index = (abs_val(seq[i] - traversal[seq_count-1]) < shortest && seq_bool[i] == false) ? i : ret_index;
        shortest = (seq[i] < shortest && seq_bool[i] == false) ? seq[i] : abs_val(seq[i] - traversal[seq_count];
    }
