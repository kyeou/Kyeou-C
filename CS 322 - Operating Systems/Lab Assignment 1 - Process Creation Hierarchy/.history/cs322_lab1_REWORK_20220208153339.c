...
void create()
{
    ...
    newC = aoc;
PCB[newC].parent = par;
if (PCB[par].children->itself == NULL) {
*(PCB[par].children->itself) = newC;
} else {
    //if "itself" is filled, it should be next, if next is filled, PCB[itself].children->itself
}
    *(PCB[par].children->itself) = aoc;
