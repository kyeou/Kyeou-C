...
bl *best_fit(int size)
{
    ...
    for (int a = 0; a < amt; a++)
    {
        if (PHY_MEM[a].start_loc != -1) {
        printf("CURR ID --> %d\n", PHY_MEM[a].id);

        // if at end of list and pm_size - last_el.end_loc > size and < ts
        if (!(PHY_MEM[a].next) && (pm_size - PHY_MEM[a].end_loc) > size && (pm_size - PHY_MEM[a].end_loc) < ts)
        {
            printf("SECOND IF TRUE (%d - %d)\n", pm_size, PHY_MEM[a].end_loc);
            return &PHY_MEM[a];
            //printf("RET SET to ID ->> %d\n", ret->id);
        }

        if ((PHY_MEM[a].next) && PHY_MEM[a].next->start_loc - PHY_MEM[a].end_loc   < ts && PHY_MEM[a].next->start_loc- PHY_MEM[a].end_loc  > size)
        {
            printf("LAST IF TRUE (%d - %d)\n", PHY_MEM[a].next->start_loc, PHY_MEM[a].end_loc);
...
void fit(bl *item, int size)
{
    ...
    if (fit_bool)
    {
        ...
        printf("BACK IN FIT CALL --> TEMP ID %d\n", temp->id);
        if (temp->id != -1)
        {
...
for (int a = 0; a < amt; a++) {
//printf("H LOOP CHECK \n");
    if (PHY_MEM[a].next->id == del->id) {
        printf("H b DEL ID CHECK --> %d\n", PHY_MEM[a].id);
        h = &PHY_MEM[a];
        //printf("H SET --------------\n");
        break;
    }
    ...
    printf("H NEXT SET\n");
    del->start_loc = -1;
    printf("DEL START LOC SET\n");
    del->end_loc = -1;
    printf("H->NEXT ID CHECK -->> %d->%d (If 2nd from last ID then -> 1)\n", h->id ,h->next != NUL);
