...
int *seq_low, *seq_high, lc = 0, hc = 0;
...
void entParms()
{
    ...
    for_cyl
    {
       if (seq[i] > seq[0]) {
           ...
           lc++;
           //PHY_MEM = (bl *)realloc(PHY_MEM, amt * sizeof(bl));
           seq_low = (int*)realloc(seq_high, lc*sizeof(int));
           seq_high[lc-1] = seq[i];
       }
...
int_input("Enter initial direction: (0=decreasing, 1=increasing):", di)


 int distance = 0;
    int seq_count = 1, high_seq_count = 0; low_seq_count = 0;
    for_cyl
    {
        seq_bool[i] = false;
    }
    ...
    while (!(all_seq()))
    {
        //if direction == false and low_seq_count < lc
        //call closestIndex(0, seq_count)
        //then increase lc, if lsc == lc -> direction = true

        //if direction == true and high_seq_count < hc
        //call closestIndex(1, seq_count)
        //then increase hc, if hsc == hc -> direction = false

    if (direction == false) {
 traversal[seq_count] = seq[closestIndex(false)];
        distance += abs_val(traversal[seq_count] - traversal[seq_count - 1]);
        seq_count++;
        low_seq_count++;
        direction = (low_seq_count == lc) ? true : direction;
    }
        

if (direction == true) {
 traversal[seq_count] = seq[closestIndex(true)];
        distance += abs_val(traversal[seq_count] - traversal[seq_count - 1]);
        seq_count++;
        high_seq_count++;
        direction = (low_seq_count == lc) ? true : direction;
