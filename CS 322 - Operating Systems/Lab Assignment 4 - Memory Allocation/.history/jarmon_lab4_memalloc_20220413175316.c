...
bl* getHead() {
    ...
    int a = INT_MAX;
    for (int i = 0; i < amt; i++) {
        a = (PHY_MEM[i].start_loc < a) ? PHY_MEM[i].start_loc : a;
        q = (PHY_MEM[i].start_loc < a) ? &PHY_MEM[i] : q; 
    }
return q;
//PHY_MEM[i]
}
...
void alloc()
{
    ...
    if (amt == 1)
    {
        printf("FIRST BLOCK SIZE -->> %d", (PHY_MEM[amt - 1]).size + PHY_MEM[amt - 1].start_loc, PHY_MEM[amt - 1].end_loc);
        //head = &(PHY_MEM[amt - 1]);
